...
@router.post("/long-term", response_model=ResponseSchema[AbsenceResponse])
def create_maternity_or_sick_leave(
    ...
    """
    result = absence_service.create_long_term_absence(db, obj_in, admin_id=admin_id)
...
def create_employee_absence(obj_in: AbsenceCreate, db: Session = Depends(get_db)):
    return ResponseSchema(data=absence_service.create_absence(db, obj_in))
...
@router.patch("/{absence_id}/approve", response_model=ResponseSchema[AbsenceResponse])
